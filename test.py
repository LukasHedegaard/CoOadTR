...
def test_one_epoch(
    ...
    for (
        ...
        class_h_target_val,
        dec_target,
    ) in metric_logger.log_every(data_loader, 500, header):
        ...
        if args.distributed:
            ...
            targets_gather_list = [
                torch.zeros_like(class_h_target) for _ in range(nprocs)
            ]
            ...
            all_probs.extend(
                enc_score_p0[:, :21].detach().cpu().numpy()
            )  # (89, 21)  # all_probs += list(prob_val)
